...
pcm_sound			sound_fx[MAX_SMUSICS];    // the array of secondary sound buffers
...
for (index=0; index < MAX_SMUSICS; index++)
	{	
...
if (source_id!=-1)
    ...
    for (int id=0; id < MAX_SMUSICS; id++)
        {
        // is this sound open?
...
if (first_time)
	...
	memset(sound_fx,0,sizeof(pcm_sound)*MAX_SMUSICS);
...
for (int index=0; index<MAX_SMUSICS; index++)
	{
