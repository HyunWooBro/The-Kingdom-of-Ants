...
for (index=0; index<bob->num_frames; index++)
    ...
    color_key.dwColorSpaceLowValue  = _RGB32BIT(0, 0, 0, 0);
    color_key.dwColorSpaceHighValue = _RGB32BIT(0, 0, 0, 0);
...
// extract bitmap data
source_ptr = bitmap->buffer + (cy*bitmap->bitmapinfoheader.biWidth+cx)*BYTE_PER_PIXER;
...
for (int index_y=0; index_y<bob->height; index_y++)
    ...
    memcpy(dest_ptr, source_ptr,bob->width*BYTE_PER_PIXER);

    // advance pointers
    dest_ptr   += (bob->width+bob->width_fill)*BYTE_PER_PIXER;
    source_ptr += bitmap->bitmapinfoheader.biWidth*BYTE_PER_PIXER;
    } // end for index_y
...
if (FAILED(lpdd_temp->QueryInterface(IID_IDirectDraw7,
                               (LPVOID *)&lpdd)))
   return(0);
...
//ddsd.ddsCaps.dwCaps2 = DDCAPS2_CANRENDERWINDOWED ;
...
for (index=0; index < MAX_COLORS_PALETTE; index++)
    ...
    // set each entry
    if (index < 64) 
        palette[index].peRed = index*BYTE_PER_PIXER; 
    else           // shades of green
    if (index >= 64 && index < 128) 
        palette[index].peGreen = (index-64)*BYTE_PER_PIXER;
    else           // shades of blue
    if (index >= 128 && index < 192) 
       palette[index].peBlue = (index-128)*BYTE_PER_PIXER;
    else           // shades of grey
    if (index >= 192 && index < 256) 
        palette[index].peRed = palette[index].peGreen = 
        palette[index].peBlue = (index-192)*BYTE_PER_PIXER;
...
int Draw_Text_GDI(char *text, int x,int y,COLORREF color, LPDIRECTDRAWSURFACE7 lpdds, int size)
{
...
// allocate the memory
if (!(image->buffer = (UCHAR *)malloc(width*height*BYTE_PER_PIXER)))
   return(0);
...
image->x         = x;
image->y         = y;
image->num_bytes = width*height*BYTE_PER_PIXER;

// clear memory out
memset(image->buffer,0,width*height*BYTE_PER_PIXER);
...
source_ptr = bitmap->buffer +
      (cy*bitmap->bitmapinfoheader.biWidth+cx)*BYTE_PER_PIXER;
...
for (int index_y=0; index_y<image->height; index_y++)
    {
    // copy next line of data to destination
    memcpy(dest_ptr, source_ptr,image->width*BYTE_PER_PIXER);

    // advance pointers
    dest_ptr   += image->width*BYTE_PER_PIXER;
    source_ptr += bitmap->bitmapinfoheader.biWidth*BYTE_PER_PIXER;
    } // end for index_y
...
if (source_bitmap->attr & BITMAP_ATTR_LOADED)
   ...
   dest_addr = dest_buffer + source_bitmap->y*lpitch + source_bitmap->x*BYTE_PER_PIXER;
   ...
   for (index=0; index<source_bitmap->height; index++)
       {
       // copy the memory
       for (pixel_x=0; pixel_x<source_bitmap->width*BYTE_PER_PIXER; pixel_x++)
           {
       ...
       dest_addr   += lpitch;
       source_addr += source_bitmap->width*BYTE_PER_PIXER;
   ...
   for (index=0; index<source_bitmap->height; index++)
       {
       // copy the memory
       memcpy(dest_addr, source_addr, source_bitmap->width*BYTE_PER_PIXER);
...
a=fread(&bitmap->bitmapfileheader,1, sizeof(BITMAPFILEHEADER), file_handle);
a=ftell(file_handle);
...
// now load the bitmap file header
a=fread(&bitmap->bitmapinfoheader,1,sizeof(BITMAPINFOHEADER), file_handle);
a=ftell(file_handle);

// now load the color palette if there is one
if (bitmap->bitmapinfoheader.biBitCount == 8)
   {
   a=fread(&bitmap->palette,1,MAX_COLORS_PALETTE*sizeof(PALETTEENTRY), file_handle);
	a=ftell(file_handle);
...
//a = _lseek(file_handle,0,SEEK_CUR);
a=fseek(file_handle,-(long)(bitmap->bitmapinfoheader.biSizeImage),SEEK_END);
a=ftell(file_handle);
...
if (bitmap->bitmapinfoheader.biBitCount==8 || bitmap->bitmapinfoheader.biBitCount==16 || 
   ...
   // now read it in
   a=fread(bitmap->buffer,1,bitmap->bitmapinfoheader.biSizeImage, file_handle);
	a=ftell(file_handle);
